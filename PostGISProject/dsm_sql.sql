...
SELECT ward_name, count(ward_name) as cntwards
from baselinedata
group by ward_name
order by cntward desc;
...
/* IT IS NEVER ALLOWED to create geometry data inconsistent with the information in this table.
Therefore, in order to transform from one coordinate system to another, it's necessary to de-register
the info in this table, transform the geometry data, and then re-register the geometry column.
...
/* It seems like the flood data is overly complex, if we're just interested in any flooding. Let's dissolve that layer into a single feature.
...
Since we don't know if we'll get single- or multi-part features out of an intersection and all of
the geometries have to be of the same type, it's safest to wrap an st_multi() function around most
geometry-altering queries.

You can add the results of this query to the QGIS map using the Load as new layer option at the bottom
of the results, and set the Geometry column to geom.
