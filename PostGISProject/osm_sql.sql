...
LIMIT 50;
/*
You can immediately see that the structure of OSM data in PostGIS is not taking advantage of relational database table
structure and is wasting a ton of database space with NULL values. For this reason, do not copy whole tables of data from
the public schema to your own schema! Select only the columns and records that you need for analysis.

The columns that you see here each represent a Tag in OSM, and the OSM2PGSQL tool has selected common tags used in OSM-based cartography.
The tool can also be customized to include other less common tags. OSM users can add any tags they like to map features:
this set has just developed out of consensus among the community of OSM users.
*/

/*
QUESTION: When was this data last edited?
Working with dates and times is always difficult in computing, so let's just extract the year and summarize edits by year,
type-casting to integer to remove the .0 decimal place*/
...
/*Sometimes the quality of OSM data is judged in terms of how frequently the data has been edited, showin in the osm_version field.
Edits could be either to attributes or geometries. Summarize the average and maximum number of edits per feature in each of the four tables, e.g. with... */
...
or whether it still represents volunteered geographic knowledge. Furthermore, is the volunteered knowledge foreign, or local?
Our best guess at this type of information is from the osm_user identity. Let's try to find the top users editing Dar es Salaam
data by counting the number of edits each user has made. */
SELECT osm_user, count(osm_id) as n
FROM planet_osm_line
GROUP BY osm_user
ORDER BY n DESC;

/*We have been analyzing the four tables seperately, but with the UNION operator, they can be combined,
as long as all of the columns are indentical. For example:*/
...
SELECT COUNT(osm_id) AS n
FROM planet_osm_polygon
WHERE amenity = 'university';



/* Then, create new tables named unipoly and unipoint in your own schema containing the osm_id, way,
and name for each point university feature and each polygon university features, using the proper UTM Zone
NOTE: by default, new tables are created in your own schema, and queries look for tables in your own schema first
and in the public schema second. If you need to specify which schema to query or create a table in, add the schema
name and a period before the table name. E.g. use dumb.unipoint to find the unipoint table in dumb schema
or use public.planet_osm_line to find the planet_osm_line table in the public schema. */
...
I'm using this following the documentation, and do not know exactly why there is an extra set of parenthesis around st_dump
folled by ".geom" which is like using AS to rename the output. I presume this is because st_dump() returns a set of rows,
rather than just one. */

/* Now we have single-part buffers! We can select by location for any of the pubs intersecting our buffers,
remembering to keep the coordinate refrence systems consistent in the comparison.
Transform the smaller set of features (buffers) to match the larger set (planet_osm_polygon).
...
/* SQL is smart: it will solve the amenity = pub part of this first. What if we crafted this to require comparing all
the spatial information first? The following query took 46 seconds instead of the 2.7 seconds for the query above  */
...
SELECT DISTINCT osm_id, name
FROM planet_osm_point INNER JOIN unibuffers
ON st_intersects(planet_osm_point.way, st_transform(unibuffers.geom,4326))
WHERE planet_osm_point.amenity = 'pub';
...
SELECT DISTINCT osm_id, name
FROM planet_osm_polygon INNER JOIN unibuffers
ON st_centroid(st_intersects(planet_osm_polygon.way, st_transform(unibuffers.geom,4326)))
WHERE planet_osm_polygon.amenity = 'pub';
...
FROM 
	(SELECT DISTINCT osm_id, name FROM planet_osm_polygon INNER JOIN unibuffers
	ON st_intersects(planet_osm_polygon.way, st_transform(unibuffers.geom,4326))
	WHERE planet_osm_polygon.amenity = 'pub'
	UNION
	SELECT DISTINCT osm_id, name FROM planet_osm_point INNER JOIN unibuffers
	ON st_intersects(planet_osm_point.way, st_transform(unibuffers.geom,4326))
	WHERE planet_osm_point.amenity = 'pub') AS a
GROUP BY osm_id;
